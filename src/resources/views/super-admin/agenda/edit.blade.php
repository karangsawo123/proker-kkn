...
<div class="admin-page-header">
    ...
    <p class="admin-page-desc">Perbarui data pelaksanaan kegiatan dan dokumentasi foto.</p>
</div>
...
<div class="admin-card">
    ...
    <div class="admin-card-body">
        <form method="POST" action="{{ route('super-admin.agenda.update', $agenda->id) }}" enctype="multipart/form-data" class="admin-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select name="scope_level" id="scope_level" class="form-select @error('scope_level') is-invalid @enderror" required onchange="toggleDusunSelector(this.value)">
                        <option value="DESA" {{ old('scope_level', $agenda->scope_level) === 'DESA' ? 'selected' : '' }}>Tingkat Desa (Seluruh Warga Desa)</option>
                        <option value="DUSUN" {{ old('scope_level', $agenda->scope_level) === 'DUSUN' ? 'selected' : '' }}>Tingkat Dusun (Wilayah Tertentu)</option>
                    </select>
                    @error('scope_level')
                ...
                <div class="form-group" id="dusunSelectorGroup" style="{{ old('scope_level', $agenda->scope_level) === 'DUSUN' ? '' : 'display: none;' }}">
                    ...
                    <select name="dusun_id" id="dusun_id" class="form-select @error('dusun_id') is-invalid @enderror">
                        ...
                        @foreach($dusuns as $dusun)
                            <option value="{{ $dusun->id }}" {{ old('dusun_id', $agenda->dusun_id) == $dusun->id ? 'selected' : '' }}>
                                {{ $dusun->nama_dusun }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    <label for="tanggal_selesai" class="form-label">
                        Tanggal Selesai <span class="optional-tag">(Opsional)</span>
                    </label>
                    <input
                        type="date"
            ...
            <!-- Media / Foto Tersimpan -->
            @if($agenda->agendaMedias->isNotEmpty())
                <div class="form-section-box">
                    <h3 class="section-box-title">Media / Foto Tersimpan</h3>
                    <p class="section-box-desc">Centang kotak untuk menghapus foto yang tidak lagi diperlukan.</p>
                    <div class="saved-media-grid">
                        @foreach($agenda->agendaMedias as $med)
                            <div class="saved-media-item">
                                <img src="{{ asset('storage/' . $med->media_path) }}" alt="Foto Kegiatan" class="saved-media-img">
                                <div class="saved-media-meta">
                                    <span class="badge badge-neutral">{{ $med->media_role === 'POSTER_AWAL' ? 'Poster Pamflet' : 'Dokumentasi' }}</span>
                                    <label class="media-remove-check">
                                        <input type="checkbox" name="existing_media_remove[]" value="{{ $med->id }}">
                                        Hapus Foto
                                    </label>
                                </div>
                            </div>
            ...
            <!-- Unggah Media Baru (Repeatable) -->
            <div class="form-section-box">
                <div class="flex-between" style="margin-bottom: 0.75rem;">
                    <div>
                        <h3 class="section-box-title">Unggah Foto Tambahan</h3>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let nextIdx = container.querySelectorAll('.media-input-row').length + 50;
