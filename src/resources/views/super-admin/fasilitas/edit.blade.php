...
<div class="admin-page-header">
    <h1 class="admin-page-title">Edit Fasilitas Umum</h1>
    <p class="admin-page-desc">Perbarui informasi dan titik koordinat fasilitas umum.</p>
</div>
...
<div class="admin-card">
    ...
    <div class="admin-card-body">
        <form method="POST" action="{{ route('super-admin.fasilitas.update', $fasilitas->id) }}" enctype="multipart/form-data" class="admin-form">
            @csrf
            @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select name="dusun_id" id="dusun_id" class="form-select @error('dusun_id') is-invalid @enderror" required>
                        ...
                        @foreach($dusuns as $dusun)
                            <option value="{{ $dusun->id }}" {{ old('dusun_id', $fasilitas->dusun_id) == $dusun->id ? 'selected' : '' }}>
                                {{ $dusun->nama_dusun }}
                            </option>
                        @endforeach
                    </select>
